...
contract YieldFarmingPoolTest is Test {
    ...
    function _deployFreshPool(uint256 rewardTokenFunding)
        internal
        returns (YieldFarmingPool pool, MockToken rwd, MockToken stk)
    {
    ...
    function _setPoolActive(bytes32 poolId, bool active) internal {
        stdstore.target(address(yieldFarmingPool)).sig("pools(bytes32)").with_key(poolId).depth(5).checked_write(active);
    ...
    function testCreatePool() public {
        ...
        (
            ...
            bool isActive1
        ) = yieldFarmingPool.pools(poolId1);
        (
            address token2,
            uint256 totalStaked2,
            uint256 rewardRate2,
            uint256 lastUpdateTime2,
            uint256 rewardPerTokenStored2,
            bool isActive2
        ) = yieldFarmingPool.pools(poolId2);
